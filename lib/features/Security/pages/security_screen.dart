import 'package:finwise/core/constants/app_assets.dart';
import 'package:finwise/core/constants/app_colors.dart';
import 'package:finwise/core/functions/navigations.dart';
import 'package:finwise/core/routes/routes.dart';
import 'package:finwise/core/styles/text_styles.dart';
import 'package:finwise/core/widgets/custom_svg_picture.dart';
import 'package:finwise/core/widgets/default_app_bar.dart';
import 'package:finwise/core/widgets/my_body_view.dart';
import 'package:flutter/material.dart';
import 'package:gap/gap.dart';

class SecurityScreen extends StatelessWidget {
  const SecurityScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: DefaultAppBar(title: "Security"),
      body: MyBodyView(
        bottomSection: SingleChildScrollView(
          child: Column(
            children: [
              SecurityOption(
                icon: AppAssets.passSettings,
                title: "Change Pin",
                onPress: () {
                  pushTo(context, Routes.changePinScreen);
                },
              ),
              Gap(20),
              SecurityOption(
                icon: AppAssets.fingerprint,
                title: "Fingerprint",

                onPress: () {
                  pushTo(context, Routes.securityFingerprintScreen);
                },
              ),
              Gap(20),
              SecurityOption(
                icon: AppAssets.security,
                title: "Terms And Conditions",
                onPress: () {
                  pushTo(context, Routes.termsAndConditions);
                },
              ),
            ],
          ),
        ),
      ),
    );
  }
}

class SecurityOption extends StatelessWidget {
  const SecurityOption({
    super.key,
    required this.icon,
    required this.title,
    required this.onPress,
  });

  final String icon;
  final String title;
  final void Function()? onPress;

  @override
  Widget build(BuildContext context) {
    return ListTile(
      leading: Container(
        height: 31,
        width: 31,
        padding: EdgeInsets.all(4),
        decoration: BoxDecoration(
          color: AppColors.mainGreen,
          borderRadius: BorderRadius.circular(30),
        ),
        child: CustomSvgPicture(path: icon),
      ),
      title: Text(title, style: TextStyles.bodyMedium),

      trailing: IconButton(
        onPressed: onPress,
        icon: Icon(
          Icons.arrow_forward_ios,
          weight: 7,
          color: Theme.of(context).brightness == Brightness.dark
              ? Colors.white
              : AppColors.lettersAndIcons,
        ),
      ),
    );
  }
}
